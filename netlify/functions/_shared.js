...
function getPeriodMeta(period, date = new Date(), offset = 0) {
  ...
  if (period === "monthly") {
    ...
    if (shift) {
      ...
      return { startMs: shifted.getTime(), label: formatMonthKey(shifted, WEEK_TZ) };
    }
  ...
  if (period === "weekly") {
    ...
    if (shift) {
      const shifted = new Date(start.getTime());
      shifted.setUTCDate(shifted.getUTCDate() + shift * 7);
      return { startMs: shifted.getTime(), label: formatDateKey(shifted, WEEK_TZ) };
    }
